...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ data, onReset, onAddToCart }) => {
  ...
  return (
    ...
    >
      <div className="mb-6 flex items-start gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-slate-700 shadow-sm">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-10 shadow-sm transition-colors lg:col-span-2">
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-10">
              ...
              <div className="rounded-2xl bg-emerald-600 px-4 py-2 text-xs font-black uppercase tracking-widest text-white shadow-[0_14px_28px_-18px_rgba(22,163,74,0.55)]">
                One-Stop Basket
              </div>
            </div>
          ...
          <div className="absolute -bottom-20 -right-20 h-80 w-80 rounded-full bg-emerald-100 blur-3xl"></div>
        </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-black text-emerald-600">2 Hours Delivery</p>
                  <p className="text-[10px] font-bold uppercase text-slate-400">Instant Slot</p>
                </div>
              </div>
            </div>
          </div>
          <button onClick={onReset} className="mt-8 min-h-11 w-full rounded-xl border border-emerald-600 bg-emerald-600 py-3 text-[10px] font-black uppercase tracking-widest text-white transition duration-200 hover:bg-emerald-700">
            Scan Another
          </button>
        </div>
      </div>
        ...
        {Object.entries(groupedMeds).map(([category, meds]: [string, Medicine[]], catIdx) => (
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-emerald-600"><BadgeCheck className="h-3 w-3" />{meds.length} Matches Found</span>
            </div>
